...
## AI-Powered Lesson Authoring (Bundled Skills)

This repository includes AI Agent Skills for authoring interactive lessons, designed to work with [OpenCode](https://opencode.dev/) or compatible AI agent environments.

> 💡 **Note**: These Skills require an AI agent environment (e.g. OpenCode) to run. They are **not** Moodle plugins you install — they are instruction sets that guide an AI to generate lesson content for you.

### Bundled Skills

| Skill | Location | Purpose |
|---|---|---|
| **micp-html-authoring** | `.skills/micp-html-authoring/SKILL.md` | Generate complete mod_micp lesson packages (index.html + micp-scoring.json) |
| **ai-dynamic-course** | `.skills/ai-dynamic-course/SKILL.md` | Generate full "AI Dynamic Course" bundles (mod_micp + Cloze quiz + optional H5P) |
| **moodle-cloze-question** | `.skills/moodle-cloze-question.md` | Generate Moodle Cloze (fill-in-the-blank) questions |
| **moodle-h5p** | `.skills/moodle-h5p.md` | Create H5P interactive exercises |

### Usage (OpenCode Agent)

Once OpenCode is installed, these Skills load automatically. Just describe what you want:

```
# Create a MICP interactive lesson about photosynthesis
"帮我制作一个关于光合作用的 MICP 互动课件"

# Generate a Cloze question
"生成一个关于细胞分裂的 Cloze 题目"

# Make an H5P matching exercise
"制作一个拖拽配对题用于练习元素周期表"

# One-shot AI dynamic course
"生成一个关于'波的叠加原理'的 AI 动态课程"
```

The agent reads the Skill instructions and produces ready-to-upload ZIP packages for mod_micp.

### Standard MICP Authoring Workflow

To hand-author a MICP lesson, the core is three files:
...
<!-- Submit -->
<button id="submit-attempt"
  onclick="window.MICP.submit({ raw: { actions: actions } })">
  Submit
</button>
```
...
Full authoring spec: `.skills/micp-html-authoring/SKILL.md`.
