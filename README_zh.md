...
MICP 是一个面向低成本互动 HTML 课件的 Moodle 活动插件。

用最直接的话说：它可以把一个互动式 HTML 课件放进 Moodle，作为普通课程活动给学生使用，并把结果回写到 Moodle 成绩册。

你不需要为每个想法单独开发 Moodle 插件，也不需要把活动丢到 Moodle 外面运行，更不需要因为活动有交互就放弃成绩册和报表。
...
**MICP 让“能互动、能接进 Moodle、还能评分”的课件，制作和落地成本大幅下降。**
...
如果你想用最快的方式制作课件，可以使用配套仓库：

- MICP 课件包生成指引：`https://github.com/RainGather/moodle_micp_skill`

这个仓库是一套给 AI 助手使用的通用说明，目标是帮助 AI 生成：
...
1. 打开这个 skill 仓库，或把它交给你的 AI 助手。
2. 告诉 AI 你要教什么主题。
3. 让 AI 生成一个 MICP 课件包。
4. 把这个课件包上传到 Moodle 里的 MICP 活动。
...
你不需要懂 GitHub，也不需要会编程，核心目标只是更快拿到一个可以上传使用的互动课件包。
...
真正有吸引力的点，不只是“支持 HTML”。

真正有吸引力的点是：**那些原本太贵、太麻烦、太技术化的互动教学活动，现在更有机会在 Moodle 里落地。**
...
如果再配合 AI 辅助制作，这个成本还能进一步下降：教师或课程团队只需要描述活动想法，就更有机会快速得到一个可上传、可运行的互动课件包。
